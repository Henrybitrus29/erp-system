...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['tamper_sale'])) {
    ...
    $update_stmt = $pdo->prepare("UPDATE sales SET quantity = ?, total_price = ? WHERE id = ?");
    $update_stmt->execute([$new_quantity, $new_price, $sale_id]);
    ...
    $telegram_message .= "Sale ID: " . $sale_id . "\n";
...
$sales_query = $pdo->query("SELECT * FROM sales ORDER BY id DESC LIMIT 50");
                ...
                <?php foreach ($sales as $sale): ?>
                    <tr>
                        <form method="POST" action="admin_tamper.php">
                            ...
                            <td>
                                <input type="number" name="quantity" value="<?= htmlspecialchars($sale['quantity']) ?>" required>
                            </td>
                            <td>
                                <input type="number" name="total_price" step="0.01" value="<?= htmlspecialchars($sale['total_price']) ?>" required>
                            </td>
                            <td>
